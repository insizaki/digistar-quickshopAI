...
integrasi Groq API untuk chatbot
"""
...
from helpers.groq_client import (
    setup_groq, generate_conclusion, get_chat_response
)
...
def initialize_session_state():
    ...
    if "groq_available" not in st.session_state:
        # Cek ketersediaan Groq API
        st.session_state.groq_available = setup_groq()
...
def display_header():
    ...
    st.markdown("*Analisis sentimen lokal dari ulasan produk Tokopedia dengan bantuan Groq API*")
...
def setup_sidebar():
    ...
    with st.sidebar:
        ...
        **Cara Menggunakan:**
        ...
        # Groq API status
        st.markdown("---")
        st.subheader("🤖 Status Groq API")
        
        if st.session_state.groq_available:
            st.success("✅ Groq API siap digunakan")
        else:
            st.error("""
            ❌ Groq API tidak tersedia
            
            Pastikan:
            1. Anda sudah mengatur variabel lingkungan GROQ_API_KEY
            2. Kunci API Groq valid
            """)
...
def process_product_url(product_url, headless_mode, max_reviews):
    ...
    with scraping_container:
        ...
        # Generate conclusion with Groq API
        if st.session_state.groq_available:
            update_status("⏳ Menghasilkan kesimpulan dengan Groq API...")
            ...
            scraped_data['conclusion'] = "Untuk mendapatkan kesimpulan produk otomatis, pastikan GROQ_API_KEY dikonfigurasi."
...
def display_chatbot(product_data):
    ...
    if not st.session_state.groq_available:
        st.warning(
            "⚠️ Chatbot membutuhkan Groq API untuk berfungsi. "
            "Pastikan GROQ_API_KEY dikonfigurasi dengan benar."
        )
        return
    ...
    if st.button("Kirim Pertanyaan"):
        if user_question:
            ...
            with st.spinner("AI sedang berpikir..."):
                # Dapatkan respons dari Groq API
...
def main():
    ...
    # Display existing product data if available
        ...
        **Catatan:** 
        ...
        - Untuk menggunakan chatbot, pastikan GROQ_API_KEY dikonfigurasi dan valid
        """)
